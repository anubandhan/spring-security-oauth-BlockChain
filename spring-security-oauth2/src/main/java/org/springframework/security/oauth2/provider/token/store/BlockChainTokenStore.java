...
import org.springframework.security.oauth2.blockchain.util.PendingAuthorisationContainer;
 ...
 */
    ...
    //TODO: We need to validate Signature and Key before setting that up in Blockchain. That's part of Authentication. FOR POC purpose we are skipping that for now.
    // TODO: Validation of refresh Token..
    @Component
public class BlockChainTokenStore implements TokenStore {

    private static final Log logger = LogFactory.getLog(BlockChainTokenStore.class);

    private static final int DEFAULT_FLUSH_INTERVAL = 1000;

    // TODO: Whats is this used for? Do we need blockchain for this?
    private final ConcurrentHashMap<String, OAuth2AccessToken> accessTokenStore = new ConcurrentHashMap<String, OAuth2AccessToken>();

    private final ConcurrentHashMap<String, OAuth2AccessToken> blockChainAccessTokenStore = new ConcurrentHashMap<String, OAuth2AccessToken>();

    //TODO: we need to modify both authenticationToAccessTokenStore and accessTokenStore. accessTokenStore will be actual blockchain amd
    // TODO : We can use authenticationToAccessTokenStore
    ...
    // This is only used when there is a client Authentication, So mostprobably we will be accessing Token mostly on User ID.
    // Note: Below we have Collection<OAuth2AccessToken> which means that against a UserName and Clein ID there can be multiple Tokens.
    // Also note that we will not require to modify this as part of our POC.
    ...
    // Below will not be required to change for this POC. It's out of scope.
    ...
    //List of Authentication Object (User was authenticated or not)
    ...
    //TODO: This method below is not required.
    /**
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        // Blockchain: Add This AccessToken to pending list. Input (Client ID, User ID and token)
        // TODO: Blockchain: Final implementation will also have Publisher ID so that multiple publishers can link with same Blockchain.
        // TODO: Blockchain: Final Implementation UserID will get replaced by Public UserID, Remember it will be different from Publsiher's User ID.


        // TODO: This is throwing a null pointer exception.

         PendingAuthorisationContainer.getInstance().addAccessToken(authentication.getOAuth2Request().getClientId(), getApprovalKey(authentication), token);
        // Blockchain : Transmit AccessToken to Peer list so that it can get published.
